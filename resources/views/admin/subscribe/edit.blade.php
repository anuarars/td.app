...
@section('content')
<div class="col-md-12">
    <form action="{{route('admin.subscribe.update', $subscribe)}}" method="post">
        @method('PUT')
        @csrf
        <div class="form-group row mt-2 mb-2">
            <label class="col-md-3" for="subscribe_name">Название подписки</label>
            <div class="col-md-8">
                <input type="text" class="form-control" name="name" placeholder="Название подписки" id="subscribe_name" value="{{$subscribe->name}}">
            </div>
        </div>
        <div class="form-group row mt-2 mb-2">
            <label class="col-md-3" for="subscribe_desc">Описание подписки</label>
            <div class="col-md-8">
                <textarea class="form-control" name="description" placeholder="Описание подписки" id="subscribe_desc" rows="6">{{$subscribe->description}}</textarea>
            </div>
        </div>
        <div class="form-group row mt-2 mb-2">
            <label class="col-md-3">Стоимость</label>
            <div class="col-md-8">
                <input type="number" class="form-control" name="price" placeholder="Стоимость в тенге" value="{{$subscribe->price}}">
            </div>
        </div>
        <div class="form-group row mt-2 mb-2">
        ...
        <div class="form-group mt-2 d-flex justify-content-center">
            <button class="btn btn-success">Обновить тип подписки</button>
        </div>
    </form>
</div>
@endsection